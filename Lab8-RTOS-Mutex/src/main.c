...
SemaphoreHandle_t xMutexLVGL;  // variável global que irá servir de mutex
...
static void task_lcd(void *pvParameters) {
	...
	for (;;)  {
		xSemaphoreTake(xMutexLVGL, portMAX_DELAY); // requisicao do mutex
		lv_tick_inc(50);
		lv_task_handler();
		xSemaphoreGive( xMutexLVGL ); // liberacao do mutex
		vTaskDelay(50);
	}
}
